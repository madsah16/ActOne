...
# ---------------------------------------------------------------------
# ActOne IDE for DAT233 startup script
# ---------------------------------------------------------------------
# Generated by MPS

message()
{
  TITLE="Cannot start ActOne IDE for DAT233"
...
# Try (in order): ActOneIDE_JDK, actoneide.jdk, ../jre, JDK_HOME, JAVA_HOME, "java" in PATH.
# ---------------------------------------------------------------------
if [ -n "$ActOneIDE_JDK" -a -x "$ActOneIDE_JDK/bin/java" ]; then
  JDK="$ActOneIDE_JDK"
elif [ -s "$HOME/.ActOneIDE1.0/config/actoneide.jdk" ]; then
  JDK=`"$CAT" $HOME/.ActOneIDE1.0/config/actoneide.jdk`
  if [ ! -d "$JDK" ]; then
    JDK="$IDE_HOME/$JDK"
  fi
...
if [ -z "$JDK" ] || [ ! -x "$JAVA_BIN" ]; then
  message "No JDK found. Please validate either ActOneIDE_JDK, JDK_HOME or JAVA_HOME environment variable points to valid JDK installation."
  exit 1
fi
...
# ---------------------------------------------------------------------
if [ -n "$ActOneIDE_PROPERTIES" ]; then
  IDE_PROPERTIES_PROPERTY="-Didea.properties.file=$ActOneIDE_PROPERTIES"
fi
...
elif [ -r "$HOME/.ActOneIDE1.0/config/mps$BITS.vmoptions" ]; then
  # user-overridden
  VM_OPTIONS_FILE="$HOME/.ActOneIDE1.0/config/mps$BITS.vmoptions"
elif [ -r "$IDE_BIN_HOME/actone$BITS.vmoptions" ]; then
  # default, standard installation
  VM_OPTIONS_FILE="$IDE_BIN_HOME/actone$BITS.vmoptions"
else
  # default, universal package
  test "$OS_TYPE" = "Darwin" && OS_SPECIFIC="mac" || OS_SPECIFIC="linux"
  VM_OPTIONS_FILE="$IDE_BIN_HOME/$OS_SPECIFIC/actone$BITS.vmoptions"
fi
...
if [ "$IS_EAP" = "true" ]; then
  ...
  if [ -r "$IDE_BIN_HOME/lib$AGENT_LIB.so" ]; then
    AGENT="-agentlib:$AGENT_LIB=disablealloc,delay=10000,sessionname=ActOneIDE1.0"
  fi
fi
...
if [ -n "$ActOneIDE_CLASSPATH" ]; then
  CLASSPATH="$CLASSPATH:$ActOneIDE_CLASSPATH"
fi
...
IDEA_PATHS_SELECTOR=ActOneIDE1.0
