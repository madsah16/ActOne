...
IF "%IDE_BIN_DIR:~-8%" == "bin\win\" (
  echo.
  echo    You are trying to run ActOne IDE for DAT233 from generic distribution
  echo Please first copy content of "%IDE_BIN_DIR%"
  echo into its parent directory ("%IDE_BIN_DIR:~0,-4%"^)
  echo and then run actone.bat from there
  echo.
  pause
  exit
)
...
:: Try (in order): ActOneIDE_JDK, actone%BITS%.exe.jdk, ..\jre, JDK_HOME, JAVA_HOME.
:: ---------------------------------------------------------------------
SET JDK=

IF EXIST "%ActOneIDE_JDK%" SET JDK=%ActOneIDE_JDK%
IF NOT "%JDK%" == "" GOTO check

SET BITS=64
SET USER_JDK64_FILE=%USERPROFILE%\.ActOneIDE1.0\config\actone%BITS%.exe.jdk
SET BITS=
SET USER_JDK_FILE=%USERPROFILE%\.ActOneIDE1.0\config\actone%BITS%.exe.jdk
...
IF NOT EXIST "%JAVA_EXE%" (
  ECHO ERROR: cannot start ActOne IDE for DAT233.
  ECHO No JDK found. Please validate either ActOneIDE_JDK, JDK_HOME or JAVA_HOME points to valid JDK installation.
  ECHO
  EXIT /B
)
...
:: ---------------------------------------------------------------------
IF NOT "%ActOneIDE_PROPERTIES%" == "" SET IDE_PROPERTIES_PROPERTY="-Didea.properties.file=%ActOneIDE_PROPERTIES%"

:: explicit
SET VM_OPTIONS_FILE=%ActOneIDE_VM_OPTIONS%
...
IF NOT EXIST "%VM_OPTIONS_FILE%" (
  :: user-overridden
  SET VM_OPTIONS_FILE=%USERPROFILE%\.ActOneIDE1.0\config\mps%BITS%.exe.vmoptions
)
IF NOT EXIST "%VM_OPTIONS_FILE%" (
  :: default, standard installation
  SET VM_OPTIONS_FILE=%IDE_BIN_DIR%\actone%BITS%.exe.vmoptions
)
IF NOT EXIST "%VM_OPTIONS_FILE%" (
  :: default, universal package
  SET VM_OPTIONS_FILE=%IDE_BIN_DIR%\win\actone%BITS%.exe.vmoptions
)
IF NOT EXIST "%VM_OPTIONS_FILE%" (
...
SET IDEA_PATHS_SELECTOR=ActOneIDE1.0
